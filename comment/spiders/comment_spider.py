...
class CommentSpider(scrapy.Spider):
    ...
    def __init__(self):
        ...
        self.header = "//div[@class='header']/div/text()"
        self.comment_text = "/p/text()"
        self.positive = "//div[@class='c-comments__evaluation']/div[@class='c-comments__evaluation-positive']/ul/li/text()"
        self.negative = "//div[@class='c-comments__evaluation']/div[@class='c-comments__evaluation-negative']/ul/li/text()"
        ...
        if response.status == 200:
            ...
            if title:
                ...
                for i, comment in enumerate(comments):
                    header = comment.xpath(self.header).get()
                    comment_text = comment.xpath(self.comment_text).get()
                    positive = comment.xpath(self.positive).get()
                    negative = comment.xpath(self.negative).get()
                ...
                all_pages = response.xpath(self.all_pages).get()
                if all_pages:
